...
# Hàm để trích in4 từ feature In4_project
...
# Hàm để chuyển đổi chuỗi thành tổng số ngày
def convert_to_days(text):
    # Tìm các giá trị năm, tháng, ngày
    years = re.search(r"(\d+)\s*năm", text)
    months = re.search(r"(\d+)\s*tháng", text)
    days = re.search(r"(\d+)\s*ngày", text)
    
    # Chuyển đổi các giá trị tìm được thành số nguyên, nếu không có thì coi là 0
    total_days = 0
    if years:
...
def preprocess_jobs(file_path_in, file_path_out):
    # # Xử lí file job post
    #đọc file new job posts 
    df = pd.read_csv(file_path_in)
    if df.shape[0] == 0:
        columns =   [
                    "Project ID", "Type", "Title", "Services", "Skills", "Description", 
                    "Posted Date", "Remaining Days", "Location", "Minimum Budget", 
                    "Maximum Budget", "Working Type", "Payment Type", "Num_applicants", 
    ...
    df["Duration (ngày)"] = df["Duration (ngày)"].apply(convert_to_days)
...
def main():
    ...
    file_in= "../../../data/raw/vlance_jobs_{}.csv".format(args.post_prefix)
    file_out= "../../../data/processed/vlance_jobs_{}.csv".format(args.post_prefix)
    preprocess_jobs(file_in, file_out)
